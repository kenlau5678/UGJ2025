...
There are separate shaders for the Built-in RP, HDRP, and URP. Each has its own unitypackage archive. Import whichever version you need.

URP and HDRP have (identical) shader graphs. The unlit version works with Unity UI. Neither RP supports lit UI shaders.

The built-in RP has an shaders based on Unity's Standard shaders, for both metallic and specular workflows.
It also has a UI shader that supports lighting.
The example scenes for built-in are more complex and includes a lit UI, but these don't work for the other RPs.

See the "SDF Shaders" PDF for documentation. These describe the built-in shaders, but most settings apply to the HDRP and URP versions as well.
...
2.0.1
Change: Removed obsolete UIMaterialLink component script.
Change: Each RP now has its own unitypackage containing exactly what it needs, including example scenes.
Upgrade Notice: All assets for built-in RP and its examples can be removed from the main package.
